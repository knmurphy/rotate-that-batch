...
if __name__ == "__main__":
    ...
    selected_folder = directories[
        folder_choice - 1
    ].strip()  # Still need to strip whitespace
    print(f"Selected folder: '{selected_folder}'")  # Debug line with quotes
    print(
        f"Full path to selected folder: {os.path.abspath(selected_folder)}"
    )  # Print full path
    ...
    print(
        f"You selected folder number {folder_choice}: '{directories[folder_choice - 1]}'"
    )
