...
@app.command()
def main(
    directory: str = typer.Option(get_config_value('default_directory', '.'), help="Directory containing videos"),
    angle: int = typer.Option(int(get_config_value('default_angle', 90)), help="Rotation angle (90, 180, or 270)"),
    preview: bool = typer.Option(False, help="Preview rotations without applying changes"),
    output: str = typer.Option(get_config_value('output_directory'), help="Output directory for rotated videos")
):
