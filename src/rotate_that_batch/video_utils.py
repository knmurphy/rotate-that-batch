...
def list_directories() -> List[str]:
    ...
    logger.info(f"Found {len(directories)} directories in the current working directory.")
    return directories
...
def preview_rotations(video_files: List[str], angle: int) -> None:
    ...
    for video_file in video_files:
        try:
            ...
            subprocess.run([
                'ffmpeg', '-i', video_file, '-vf', f'rotate={angle}*PI/180', 
            ...
            logger.error(f"Failed to generate preview for {video_file}. Error: {e.stderr.decode()}")
        except Exception as e:
